        ...
        <form class="pt-16 md:pt-20">
            <div class="px-2 md:px-4">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-6 xl:gap-8">
                    <div v-for="product in productListsx" :key="product.id"
...
export default {
    ...
    methods: {
        async fetchProductData() {
            try {
                const response = await Api.get('productsx');
                this.productListsx = response.data.data.map(product => ({
                    ...product,
                    inputValue: 0,
                }));
            } catch (error) {
        ...
        addToBag(product) {
            if (!this.userLoggedIn) {
                ...
                if (!existingProduct) {
                    ...
                    product.inputValue = this.productListsx.price; // Atau atur ke nilai default
                    this.total += 1;
